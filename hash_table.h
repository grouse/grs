...
template<typename K, typename V>
void map_reset(HashTable<K, V> *table)
{
    table->slots = nullptr;
    table->count = 0;
    table->capacity = 0;
...
template<typename V>
void map_reset(HashTable<String, V> *table, Allocator mem)
{
    if (!table->alloc) table->alloc = mem;
    PANIC_IF(table->alloc != mem, "mismatching allocators");
...
template<typename K, typename V>
void map_remove(HashTable<K, V> *table, K key)
{
    i32 slot = find_slot(table, key);
    if (slot == -1 || !table->slots[slot].occupied) return;
    table->slots[slot].occupied = false;
    table->count--;
}
