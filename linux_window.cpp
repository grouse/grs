...
{
	if (!g_mouse.captured) return;
	XUngrabPointer(x11.dsp, CurrentTime);
...
static void init_x11()
{
	...
	if (!x11.WM_PROTOCOLS) {
        X11_GET_ATOM(x11.dsp, WM_PROTOCOLS);
...
static void init_cursors(Window wnd)
{
    if (!cursors[MC_NORMAL]) {
        ...
        Pixmap csr = XCreateBitmapFromData(x11.dsp, wnd, csr_bits, 1, 1);
        cursors[MC_HIDDEN] = XCreatePixmapCursor(x11.dsp, csr, csr, &xcolor, &xcolor, 1, 1);
    }
}
...
static void init_clipboard()
{
    if (!clipboard.init) {
        clipboard.wnd = XCreateSimpleWindow(
            x11.dsp,
            RootWindow(x11.dsp, DefaultScreen(x11.dsp)),
            -10, -10,
            1, 1,
            0, 0,
            0);
