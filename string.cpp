...
String stringf(Allocator mem, const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    result.data = ALLOC_ARR(mem, char, length+1);
...
String string_from_utf16(const u16 *in_str, i32 length, Allocator mem)
{
    ...
    String str = {};
    str.data = ALLOC_ARR(mem, char, capacity);
    ...
    for (const u16 *ptr = in_str; ptr < in_str+length; ptr++) {
        ...
        if (utf32 <= 0x7F) {
            if (str.length + 1 > capacity) {
                ...
                str.data = REALLOC_ARR(mem, char, str.data, old_capacity, capacity);
            }
...
u16* utf16_from_string(String str, i32 *utf16_length, Allocator mem) EXPORT
{
    ...
    u16 *utf16 = ALLOC_ARR(mem, u16, str.length);
    ...
    for (i32 i = 0; i < str.length; i++) {
            ...
            (c >= 0xE000 && c <= 0xFFFF))
        {
            if (length+1 > capacity) {
                i32 old_capacity = capacity;
                capacity = length+1;
                utf16 = REALLOC_ARR(mem, u16, utf16, old_capacity, capacity);
            }
...
String join_path(String root, String filename, Allocator mem) EXPORT
{
    ...
    String path;
    path.data = ALLOC_ARR(mem, char, required_length);
...
char* join_path(const char *sz_root, const char *sz_filename, Allocator mem) EXPORT
{
    ...
    char *sz_path = ALLOC_ARR(mem, char, required_length+1);
...
bool parse_cmd_argument(String *args, i32 count, String name, f32 values[2]) EXPORT
{
    for (i32 i = 0; i < count; i++) {
        if (args[i] == name) {
            if (i+2 >= count) return false;
            if (!f32_from_string(args[1], &values[0])) return false;
            if (!f32_from_string(args[2], &values[1])) return false;
            return true;
        }
    }
